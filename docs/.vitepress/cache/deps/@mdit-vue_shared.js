...
var resolveTitleFromToken = (token, { shouldAllowHtml, shouldEscapeText }) => {
  ...
  const titleTokens = children.filter((item) => {
    var _a;
    return titleTokenTypes.includes(item.type) && !((_a = item.meta) == null ? void 0 : _a.isPermalinkSymbol);
  });
...
var resolveHeadersFromTokens = (tokens, { level, shouldAllowHtml, shouldAllowNested, shouldEscapeText, slugify: slugify$1, format }) => {
  const headers = [];
  const stack = [];
  const push = (header) => {
